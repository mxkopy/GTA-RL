...
{
    inline static StructuredMemory<VertexShaderConstants> VSMemory{ "VSConstantBuffers" };
...
#define MAP_VSB_OFFSET(NAME, VSB_INDEX, VSB_OFFSET) \
    ...
    static void Update(ComPtr<ID3D11DeviceContext> DeviceContext)
    {
        ...
        DeviceContext->VSGetConstantBuffers(L, R - L, Buffers.data());
...
inline static Matrix3f& Axes()
{
    static Matrix3f M;
    Eigen::Map<Vector3f> R = VSConstantBuffers::R, L = VSConstantBuffers::L, D = VSConstantBuffers::D, U = VSConstantBuffers::U;
